...
class UnitTestLogger : public emptycpp::utils::ILogger {
public:
  static bool verbose;
  UnitTestLogger() {}
  ~UnitTestLogger() {}

  void message(emptycpp::utils::LOG_MESSAGE_KIND kind, const std::string &msg) {
    std::stringstream ss;
    switch (kind) {
    case emptycpp::utils::LOG_MESSAGE_KIND::FATAL:
      ss << "[err] " << msg << std::endl;
      break;
    case emptycpp::utils::LOG_MESSAGE_KIND::INFO:
      ss << "[inf] " << msg << std::endl;
      break;
    case emptycpp::utils::LOG_MESSAGE_KIND::MESSAGE:
      ss << "[dbg] " << msg << std::endl;
      break;
    }
    if (kind == emptycpp::utils::LOG_MESSAGE_KIND::FATAL) {
      std::cerr << ss.str();
    } else {
      if (verbose) {
        std::cout << ss.str();
...
struct LoggerControl : Catch::TestEventListenerBase {
  ...
  virtual void testCaseStarting(Catch::TestCaseInfo const &) override {
    ...
    _logger = emptycpp::utils::ILogger_ptr{_raw_ptr};
  ...
  virtual void testCaseEnded(Catch::TestCaseStats const &testCaseStats) override {
    ...
    emptycpp::utils::LogManager::stop();
    _logger = nullptr;
  }
  UnitTestLogger *_raw_ptr;
  emptycpp::utils::ILogger_ptr _logger;
};

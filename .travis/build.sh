...
if [ "$CLANG" == "FALSE" ]; then
	...
	if [[ -z "${SANITIZER}" ]]; then
		...
		cmake -DCMAKE_BUILD_TYPE=RELWITHDEBINFO -DBoost_USE_STATIC_LIBS=ON -DEMPTYCPP_ENABLE_DOUBLECHECKS=OFF -DBoost_USE_MULTITHREADED=ON  -DBoost_USE_STATIC_RUNTIME=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS_RELEASE="${CMAKE_CXX_FLAGS_RELEASE}"  -DEMPTYCPP_ENABLE_BENCHMARKS=ON  -DCMAKE_EXE_LINKER_FLAGS="${CMAKE_EXE_LINKER_FLAGS}"  .
            fi
